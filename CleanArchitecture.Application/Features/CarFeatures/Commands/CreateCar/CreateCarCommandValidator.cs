...
public sealed class CreateCarCommandValidator : AbstractValidator<CreateCarCommand>
{
    public CreateCarCommandValidator()
    {
        ...
        RuleFor(p=>p.Name).MinimumLength(3).WithMessage("Araç adı en az 3 karekterden oluşmalıdır");

        RuleFor(p => p.Model).NotEmpty().WithMessage("Araç modeli boş olamaz");
        RuleFor(p => p.Name).NotNull().WithMessage("Araç modeli boş olamaz");
        RuleFor(p => p.Name).MinimumLength(3).WithMessage("Araç modeli en az 3 karekterden oluşmalıdır");
